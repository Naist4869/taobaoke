...
func NewOrder(id string, userID string, adzoneID int64, title string, itemID int64, picURL string, shopName string, shopType int, price int64, reservePrice int64, coupon int64, rebate int64, URL string, couponShareURL string, key string) *Order {
	return &Order{ID: id, UserID: userID, AdzoneID: adzoneID, Title: title, ItemID: itemID, PicURL: picURL, ShopName: shopName, ShopType: shopType, Price: price, Coupon: coupon, ReservePrice: reservePrice, Rebate: rebate, URL: URL, CouponShareURL: couponShareURL, Key: key}
}
